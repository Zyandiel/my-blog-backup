    ...
    <?php if ( ! empty( $links ) ) : ?>
        <div class="friends-grid">
            <?php foreach ( $links as $link ) : ?>
                <?php
                // Check if the latest post is within one week and glow animation is enabled
                $is_recent = false;
                $glow_animation_enabled = get_option('friends_plugin_glow_animation_enabled', 1);
                if ( ! empty( $link['latest_post_date'] ) && $glow_animation_enabled ) {
                    $post_date = strtotime( $link['latest_post_date'] );
                    $one_week_ago = strtotime( '-1 week' );
                    $is_recent = $post_date > $one_week_ago;
                }
                $card_class = $is_recent ? 'friend-card recent-post' : 'friend-card';
                ?>
                <div class="<?php echo esc_attr( $card_class ); ?>">
                    <div class="friend-card-header">
                        <a href="<?php echo esc_url( $link['url'] ); ?>" target="_blank" rel="noopener noreferrer" class="friend-icon-link">
                            <img src="<?php echo esc_url( $link['icon_url'] ); ?>" alt="<?php echo esc_attr( $link['name'] ); ?> Icon" class="friend-icon">
                        </a>
