...
final class UIL_Plugin {
	...
	public function get_settings() {
		...
		foreach ( array( 'allow_self_edit', 'show_user_column', 'auto_author_archive', 'auto_post_author', 'auto_buddypress', 'auto_bbpress' ) as $boolean_key ) {
	...
	public function render_dom_template() {
		...
		printf(
			'<template id="uil-dom-template" data-selector="%1$s" data-position="%2$s" data-user-id="%3$d"><span class="uil-auto-wrap uil-auto-wrap--selector">%4$s</span></template>',
			esc_attr( $selector ),
			esc_attr( $position ),
			$user_id,
