...
final class UIL_Admin {
	...
	public function __construct( UIL_Plugin $plugin ) {
		...
		add_action( 'admin_post_uil_save_my_labels', array( $this, 'handle_save_my_labels' ) );
	...
	public function render_admin_page() {
		...
		// Use a plugin-specific query key. Generic `tab` is commonly rewritten by
		// themes/admin plugins and caused every navigation item to show this screen.
		$tab = sanitize_key( $this->get_scalar_request_value( $_GET, 'uil_section', 'labels' ) );
		...
		<div class="wrap uil-admin-wrap">
			...
			<nav class="nav-tab-wrapper" aria-label="<?php esc_attr_e( '身份标签页面', 'user-identity-labels' ); ?>">
				<?php $base_url = admin_url( 'users.php?page=uil-identity-labels' ); ?>
				<a class="nav-tab <?php echo 'labels' === $tab ? 'nav-tab-active' : ''; ?>" <?php echo 'labels' === $tab ? 'aria-current="page"' : ''; ?> href="<?php echo esc_url( add_query_arg( 'uil_section', 'labels', $base_url ) ); ?>"><?php esc_html_e( '标签与分配', 'user-identity-labels' ); ?></a>
				<a class="nav-tab <?php echo 'settings' === $tab ? 'nav-tab-active' : ''; ?>" <?php echo 'settings' === $tab ? 'aria-current="page"' : ''; ?> href="<?php echo esc_url( add_query_arg( 'uil_section', 'settings', $base_url ) ); ?>"><?php esc_html_e( '显示设置', 'user-identity-labels' ); ?></a>
				<a class="nav-tab <?php echo 'help' === $tab ? 'nav-tab-active' : ''; ?>" <?php echo 'help' === $tab ? 'aria-current="page"' : ''; ?> href="<?php echo esc_url( add_query_arg( 'uil_section', 'help', $base_url ) ); ?>"><?php esc_html_e( '使用方法', 'user-identity-labels' ); ?></a>
			</nav>
	...
	private function render_labels_tab() {
		...
		$assigned        = $this->plugin->get_user_label_ids( $current_user_id );
		?>
		<div class="uil-admin-grid">
			...
			<section class="uil-panel uil-panel--wide">
				...
				<?php if ( ! empty( $labels ) && $current_user_id ) : ?>
					<hr class="uil-panel-separator">
					<h2><?php esc_html_e( '分配给我的账号', 'user-identity-labels' ); ?></h2>
					<p class="description"><?php esc_html_e( '创建标签只是建立标签库；必须在这里勾选并保存，标签才属于你的账号并能在前台显示。', 'user-identity-labels' ); ?></p>
					<form class="uil-my-labels-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
						<input type="hidden" name="action" value="uil_save_my_labels">
						<?php wp_nonce_field( 'uil_save_my_labels' ); ?>
						<div class="uil-assignment-grid">
							<?php foreach ( $labels as $label ) : ?>
								<label class="uil-assignment-item">
									<input type="checkbox" name="uil_label_ids[]" value="<?php echo esc_attr( $label['id'] ); ?>" <?php checked( in_array( $label['id'], $assigned, true ) ); ?>>
									<?php echo wp_kses( $this->render_admin_badge( $label ), $this->plugin->get_allowed_label_html() ); ?>
								</label>
							<?php endforeach; ?>
						</div>
						<?php submit_button( __( '保存我的身份标签', 'user-identity-labels' ), 'primary', 'submit', false ); ?>
						<a class="button" href="<?php echo esc_url( admin_url( 'users.php' ) ); ?>"><?php esc_html_e( '管理其他用户', 'user-identity-labels' ); ?></a>
					</form>
				<?php endif; ?>
			</section>
		</div>
	...
	private function render_label_form( $label = null ) {
		...
		<form class="uil-label-form" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
			...
			<?php if ( ! $label['id'] ) : ?>
				<label class="uil-inline-check">
					<input name="assign_to_current_user" type="checkbox" value="1" checked>
					<span><?php esc_html_e( '创建后同时分配给我的账号', 'user-identity-labels' ); ?></span>
				</label>
			<?php endif; ?>
	...
	private function render_settings_tab() {
		...
		<section class="uil-panel uil-settings-panel">
			<form action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>" method="post">
				...
				<label class="uil-check-row">
					<input type="checkbox" name="auto_argon_sidebar" value="1" <?php checked( $settings['auto_argon_sidebar'] ); ?>>
					<span><strong><?php esc_html_e( 'Argon 主题侧栏头像/昵称下方', 'user-identity-labels' ); ?></strong><small><?php esc_html_e( '自动适配你截图中的“站点概览”资料卡，无需填写 CSS 选择器。', 'user-identity-labels' ); ?></small></span>
				</label>
				<label class="uil-full-field" for="uil-argon-user-id">
					<span><?php esc_html_e( 'Argon 资料卡代表的用户', 'user-identity-labels' ); ?></span>
					<?php
					wp_dropdown_users(
						array(
							'name'             => 'argon_user_id',
							'id'               => 'uil-argon-user-id',
							'selected'         => absint( $settings['argon_user_id'] ),
							'show_option_none' => __( '请选择用户', 'user-identity-labels' ),
						)
					);
					?>
					<small><?php esc_html_e( 'Argon 的侧栏头像是主题设置，不会自动关联 WordPress 用户；请在这里指定它代表谁。', 'user-identity-labels' ); ?></small>
				</label>
				<label class="uil-check-row">
	...
	public function handle_save_label() {
		...
		if ( $is_new_label && ! empty( $_POST['assign_to_current_user'] ) ) {
			$user_id  = get_current_user_id();
			$assigned = $this->plugin->get_user_label_ids( $user_id );
			$assigned[] = $label_id;
			update_user_option( $user_id, UIL_Plugin::USER_META_KEY, array_values( array_unique( $assigned ) ), false );
		}
	...
	/**
	 * Saves labels assigned to the currently signed-in account from the main screen.
	 *
	 * @return void
	 */
	public function handle_save_my_labels() {
		$this->assert_manage_permission();
		check_admin_referer( 'uil_save_my_labels' );

		$user_id   = get_current_user_id();
		$submitted = isset( $_POST['uil_label_ids'] ) && is_array( $_POST['uil_label_ids'] )
			? wp_unslash( $_POST['uil_label_ids'] )
			: array();
		$known = $this->plugin->get_label_definitions( false );
		$valid = array();
		...
		$this->redirect_with_notice( 'assignments_saved' );
	}
	...
	public function handle_save_settings() {
		...
		$settings = array(
			...
			'auto_argon_sidebar'  => empty( $_POST['auto_argon_sidebar'] ) ? 0 : 1,
			'argon_user_id'       => absint( $this->get_scalar_request_value( $_POST, 'argon_user_id' ) ),
	...
	public function show_admin_notice() {
		...
		$messages = array(
			'label_saved'   => array( 'success', __( '身份标签已保存。新建标签若勾选了“同时分配给我”，现在已可在前台显示。', 'user-identity-labels' ) ),
			'assignments_saved' => array( 'success', __( '你的身份标签分配已保存。', 'user-identity-labels' ) ),
